...
in setting 1, to compare transients.
=#
...
function plot_abm(ax, s, l = false)
    ...
    Y = []; X = []
    for i in 1:length(s[:])
        if l # only obtaining individuals on vertex X 
            y = []
            for a in s[i]
                if a[1][] == 4
                    # neutral trait selected is X
                    push!(y, a[2][20])
                end
            end
        else
            y = [a[2][4] for a in s[i]]
        end
...
###################################
dim_neutr = 20
neutrspace = RealSpace{dim_neutr,Float32}()
myspace = (GraphSpace(g),neutrspace)
...
# fig.savefig("pde-vs-IBM-trans-setting1_metapop_mu==$mar.pdf",
#     dpi=500,
#     bbox_inches = "tight",)
# gcf()

########################################################
## Plotting local only local trait distrib on vertex #
########################################################
cla()
