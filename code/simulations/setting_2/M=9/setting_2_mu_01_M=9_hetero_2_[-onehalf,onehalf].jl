...
We consider setting 2 with 2 different values for habitat: [-1/2, 1/2].
...
- 2022-01-XX : tend = 1000, tsaving = tend - 99 : tend, length = 200, initial number of ind : nodes * Int(K1), α and β calculated
- 2022-02-27 : tend = 3000, tsaving = tend - 99 : tend, length = 200, initial number of ind : nodes * Int(K1), α and β calculated
...
for g in graphs
    ...
    if _s > 2
        soptim_df = soptim_df[sample([1,_s,round(Int,(_s+1)/4),round(Int,3*(_s+1)/4), round(Int,(_s+1)/2)], 2, replace=false),:]
    end
