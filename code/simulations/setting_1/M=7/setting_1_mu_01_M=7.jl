...
- 2022-01-04 : tend = 1000, tsaving = tend - 99 : tend, length = 100
- 2022-01-05 : tend = 1000, tsaving = tend - 99 : tend, length = 200
- 2022-01-07 : tend = 1000, tsaving = tend - 99 : tend, length = 200, initial number of ind : nodes * Int(K1)
- 2022-01-09 : tend = 1000, tsaving = tend - 99 : tend, length = 200, initial number of ind : nodes * Int(K1), α and γ calculated
- 2022-02-16 : tend = 2000, tsaving = tend - 99 : tend, length = 200, initial number of ind : nodes * Int(K1), α and β calculated
...
function sim(p)
    ...
    success=true
    return (mu[1][1], mu[2][1], D2, myspace[1].g, get_tend(s), mean(s["N"][2:end]), mean(s["alphau"][2:end]), mean(s["betau"][2:end]), success)
end
