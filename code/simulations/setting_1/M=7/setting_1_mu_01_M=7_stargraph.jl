...
Here we run simulations on the star graphs for SI figure pde-vs-IBM-mresponse-setting1
=# 
...
function sim(p)
    ...
    success=true
    return (mu[1][1], mu[2][1], D2, myspace[1].g, get_tend(s), mean(s["N"][2:end]), mean(s["betau"][2:end]), mean(s["gammau"][2:end]), success)
end
